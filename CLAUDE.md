...
```
/create-app create-an-ai-chat-app-with-tan-afadb5
```
...
- The host sees the preview at **https://localhost:5978**
...
Include **https://localhost:5978** in your review request broadcast so reviewers and the user can preview the app. If the HTTPS cert isn't trusted, they can use the HTTP fallback or run `pnpm trust-cert` from the Electric Studio repo root. Do NOT tell users to "click through a browser warning".
